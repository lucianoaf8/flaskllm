...
def _backend(settings) -> CacheBackend:
    if settings.cache_backend == "file":
        return FileCache(settings.cache_dir, settings.cache_max_size)
    if settings.cache_backend == "redis":
        return RedisCache(settings.redis_url)
    if settings.cache_backend == "mysql":
        if not settings.mysql_url:
            raise APIError("MySQL cache selected but mysql_url not set")
        return MySQLCache(settings.mysql_url)
    return MemoryCache(settings.cache_max_size)


def get_cache(settings) -> CacheBackend:
