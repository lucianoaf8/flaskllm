...
from core.user_settings import LLMSettings, Preference, UISettings
...
def update_llm_settings():
    ...
    return jsonify({"llm_settings": settings.llm_settings.dict()})
...
def update_ui_settings():
    ...
    return jsonify({"ui_settings": settings.ui_settings.dict()})
...
def update_preference():
    ...
    return jsonify({"preference": {"key": key, "value": value, "category": category}})
...
def delete_preference(key):
    ...
    if success:
        user_settings_storage.save_settings(settings)
        return jsonify({"message": f"Preference {key} deleted successfully"})
    else:
        return (
            jsonify(
                {
...
def update_favorite_templates():
    ...
    return jsonify({"favorite_templates": settings.favorite_templates})
